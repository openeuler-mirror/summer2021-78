...
pub(crate) fn mul_i32_result_i32<'a, T>(container_a: &'a mut T, container_b: &T) -> &'a T
    where
...
#[cfg(test)]
mod tests {
    use crate::mul::mul_i32_result_i32::mul_i32_result_i32;

    #[test]
    fn mul_i32_result_i32_works() {
        let mut a = vec![1, 2, 4, 8, 16, 32, 64, 128, 256, 512];
        let b = vec![512, 256, 128, 64, 32, 16, 8, 4, 2, 1];
        mul_i32_result_i32(&mut a, &b);
        for i in 0..10 {
            assert_eq!(a[i], 512);
        }
    }
