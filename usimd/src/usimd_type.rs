...
pub struct USimdI32<'a> {
    pub data: &'a mut Vec<i32>,
}

impl<'a> From<&'a  mut Vec<i32>> for USimdI32<'a> {
    fn from(item: &'a mut Vec<i32>) -> Self {
        USimdI32 { data: item }
    }
}
...
unsafe fn add_avx2_ref<'a>(a: &USimdI32<'a>, b: &USimdI32<'a>) -> Vec<i32> {
...
unsafe fn add_avx2<'a>(a:USimdI32<'a>, b: USimdI32<'a>) -> USimdI32<'a> {
    ...
    for i in 0..group {
        ...
        let b_vector = _mm256_loadu_epi32(&b.data[i * 8] as *const i32);
        _mm256_store_epi32(
            &mut a.data[i * 8] as *mut i32,
            _mm256_add_epi32(a_vector, b_vector),
        );
    }
    for i in 0..len_a - group * 8 {
        a.data[group * 8 + i] = a.data[group * 8 + i] + b.data[group * 8 + i];
    }
    USimdI32::from(a.data)
}
