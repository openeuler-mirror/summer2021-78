...
pub(crate) fn add_i16<'a, T>(container_a: &'a mut T, container_b: &T) -> &'a T
    where
        T: IndexMut<usize, Output = i16> + len_trait::Len + ?Sized,
{
    return if is_x86_feature_detected!("avx512f") {
        unsafe { add_i16_avx512(container_a, container_b) }
    } else if is_x86_feature_detected!("avx") {
        unsafe { add_i16_avx(container_a, container_b) }
    } else if is_x86_feature_detected!("sse") {
        unsafe { add_i16_sse(container_a, container_b) }
    } else {
        add_i16_without_simd(container_a, container_b)
    };
}
#[cfg(target_arch = "arm")]
...
#[cfg(any(target_arch = "x86", target_arch = "x86_64"))]
unsafe fn add_i16_avx512<'a, T>(container_a: &'a mut T, container_b: &T) -> &'a T
    where
        T: IndexMut<usize, Output = i16> + len_trait::Len + ?Sized,
{
    ...
    for i in 0..group {
        ...
        _mm512_storeu_epi16(
            ...
            _mm512_add_epi16(a_vector, b_vector),
        );
    }
...
#[cfg(any(target_arch = "x86", target_arch = "x86_64"))]
unsafe fn add_i16_avx<'a, T>(container_a: &'a mut T, container_b: &T) -> &'a T
    where
        T: IndexMut<usize, Output = i16> + len_trait::Len + ?Sized,
{
    let len_a = container_a.len();
    ...
    for i in 0..group {
        ...
        _mm256_storeu_epi16(
            &mut container_a[i * group_len] as *mut i16,
            _mm256_add_epi16(a_vector, b_vector),
        );
    }
    for i in 0..len_a - group * group_len {
        container_a[group * group_len + i] = container_a[group * group_len + i] + container_b[group * group_len + i];
...
#[cfg(any(target_arch = "x86", target_arch = "x86_64"))]
unsafe fn add_i16_sse<'a, T>(container_a: &'a mut T, container_b: &T) -> &'a T
    where
        T: IndexMut<usize, Output = i16> + len_trait::Len + ?Sized,
{
    let len_a = container_a.len();
    ...
    for i in 0..group {
        ...
        _mm_storeu_epi16(
            ...
            _mm_add_epi16(a_vector, b_vector),
        );
    }
    for i in 0..len_a - group * group_len {
        container_a[group * group_len + i] = container_a[group * group_len + i] + container_b[group * group_len + i];
...
fn add_i16_without_simd<'a, T>(container_a: &'a mut T, container_b: &T) -> &'a T
    where
        T: IndexMut<usize, Output = i16> + len_trait::Len + ?Sized,
{
    let len_a = container_a.len();
